...
class P2PProtocol:
    ...
    async def handle_message(self, message, writer):
        message_handlers = {
            ...
            "transaciton": self.handle_transaction,
        }

        handler = message_handlers.get(message["name"])
        if not handler:
            return P2PError("Missing handler for message")
    ...
    async def handle_ping(self, message, writer):
        ...
        peers_message = create_peers_message(
            self.server.get_external_ip,
            self.server.get_external_port,
            peers)
    ...
    async def handle_transaction(self, message, writer):
        ...
        if validate_transaction(tx) is True:
            ...
            for peer in self.connection_pool.get_alive_peers(20):
                await self.send_message(
                    peer,
                    create_transaction_message(self.server.get_external_ip, self.server.get_external_port, tx),
                )
        else:
    ...
    async def handle_block(self, message, writer):
        ...
        for peer in self.connection_pool.get_alive_peers(20):
            await self.send_message(
                peer,
                create_block_message(self.server.get_external_ip, self.server.get_external_port, block),
            )
